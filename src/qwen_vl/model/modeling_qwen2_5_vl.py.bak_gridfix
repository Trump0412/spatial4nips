...
class Qwen2_5_VLForConditionalGenerationWithVGGT(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def _process_geometry_features(self, image_embeds, geometry_encoder_inputs, inputs_embeds):
        ...
        if geo_embeds is not None:
            ...
            if getattr(self.config, 'depart_smi_token', False) and n_image > getattr(self.config, 'smi_image_num', 8): 
                ...
                smi_downsample_rate = getattr(self.config, 'smi_downsample_rate', 2)
                image_embeds = image_embeds.view(n_image, h_patch, w_patch, geo_embeds.shape[-1])
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                if labels is not None:
                    text_embeds = inputs_embeds[(vision_mask == False) & (labels == -100)].view(1,-1,self.config.hidden_size)
                ...
                if getattr(self.config, 'use_geometry_encoder', False) and geometry_encoder_inputs is not None:
                    image_embeds, geo_embeds = self._process_geometry_features(image_embeds, geometry_encoder_inputs, text_embeds)
