#!/bin/bash
# 7B Vanilla: ZenView (DA3-new output + MSGF + temporal bonus + atom refinement)
set -euo pipefail
...
OUTPUT_DIR=${OUTPUT_DIR:-"${PROJECT_ROOT}/outputs/vanilla_qwen25vl_7b_da3_msgf_temporal_refine"}
LOG_DIR=${LOG_DIR:-"${PROJECT_ROOT}/logs/vanilla_qwen25vl_7b_da3_msgf_temporal_refine"}
TRAIN_LOG=${TRAIN_LOG:-"${LOG_DIR}/train.log"}
BASE_EXTRA_ARGS="--geo_inject_version da3_msgf_temporal_refine --msgf_topr 32 --msgf_frame_topk_max 3 --msgf_atom_topk_max 8 --msgf_use_bidirectional True --temporal_bonus_lambda 0.10"
